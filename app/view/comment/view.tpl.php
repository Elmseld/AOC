<div id='comment-<?=$comment->getProperties()['id']?>' class='anchor'></div>
    ...
    <div class="comment-section">
        ...
        <p><span id='smallerText'><a href='<?=$this->url->create('users/id').'/'.$comment->user->getProperties()['id']?>'><?=$comment->user->getProperties()['acronym']?></a></span>
            <span id='smallerText'>|
                ...
                <?php if (isset($comment->getProperties()['updated'])) : ?>
                    | <span id='smallerText'> Uppdaterad för
                ...
                <?php if (!$vote): ?>
                    ...
                    <?php else : ?>
                        ...
                        &nbsp;<span class='downvote lowered-letter'><i id="commentVoted" class="fa fa-caret-down fa-2x"></i></span>
                    <?php endif; ?>
            </span>
            <?php if ($this->di->LoginController->checkLoginCorrectUser($comment->user->getProperties()['id'])): ?>
                &nbsp;<a id='smallerText' href='<?=$this->url->create("{$this->request->getRoute()}?editcomment=yes&amp;commentid=".$comment->getProperties()['id']."#comment-editform-container")?>' title='Redigera'><i class="fa fa-pencil"></i> Redigera</a>
            <?php endif; ?>
        </p>
    </div>
    </div> <!-- comment-section -->
