...
class AnswerController implements \Anax\DI\IInjectionAware {
     ...
     * @return void
     */
    public function listAction($question) {
        ...
        foreach ($all as $answer_post) {
            ...
            if ($this->request->getGet('editanswer') && $this->request->getGet('answerid') === $answer_post->getProperties()['id']) {
                if (!$this->di->LoginController->checkLoginCorrectUser($answer_post->getProperties()['answerUserId'])) {
                    // Not logged in
                    $this->di->LoginController->redirectToLogin('Endast '.$answer_post->user->getProperties()['acronym'].' kan redigera svaret');
                }
                $editform = true;
                $answerform = false;
        ...
        if ($this->request->getGet('newanswer')) {
            if ($this->di->LoginController->checkLoginSimple()) {
                ...
                // Not logged in
                $this->di->LoginController->redirectToLogin();
            }
    ...
    private function add($questionId = null) {
        ...
        $form = new \Enax\HTMLForm\EFormAddAnswer($questionId);
        $form->setDI($this->di);
        $form->check();
    ...
    private function edit($answer = null) {
        ...
        $form = new \Enax\HTMLForm\EFormUpdateAnswer($answer);
        $form->setDI($this->di);
        $form->check();
    ...
    */
     ...
     */
     public function acceptAction($answerId) {
         ...
         if (!$this->di->LoginController->checkLoginCorrectUser($checkuser[0]->questionUserId)) {
             // Not logged in
             $this->di->LoginController->redirectToLogin('Logga in som rätt användare');
         }
         ...
         $this->di->msgFlash->success('<p><span class="flashmsgicon"><i class="fa fa-check-circle fa-2x"></i></span> Accepterat svar</p>');
      ...
      */
      public function unacceptAction($answerId) {
         ...
         $this->di->msgFlash->info('<p><span class="flashmsgicon"><i class="fa fa-check-circle fa-2x"></i></span> Ångrade accepterat svar</p>');
         $url = $this->url->create('question/id/'.$ans->questionId);
         $this->response->redirect($url);
      }
